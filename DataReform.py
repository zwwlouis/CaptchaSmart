...
def fetch_data(dataSet, sampleNum=10, evt_filter=(Constants.MOUSE_MOVE_EVENT, Constants.MOUSE_DOWN_EVENT)):
    """
    从数据库查询所得中提取所需的训练数据
    ...
    for item in dataSet:
        temp = item[0]
        opline = reform(item[0], sampleNum, evt_filter)
        # 对op_str数据进行reform操作
        if len(opline) > 0:
            input.append(reform(item[0], sampleNum, evt_filter))
            intLabel = int(item[2])
...
def testdata_cut(inData, inLabel, rate):
    """
    ...
    for i in range(len(inData)):
        ...
        if rn < rate:
            testinput.append(inData[i])
            testlabel.append(inLabel[i])
    # 从训练集中删除测试数据
    for i in range(len(testinput)):
        inData.remove(testinput[i])
        inLabel.remove(testlabel[i])
    entry = {
        'testinput': testinput,
        'testlabel': testlabel,
    }
